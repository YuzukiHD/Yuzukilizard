...
static int ParseCmdLine(int argc, char **argv,
                        SampleVIRVI2VOZoomCmdLineParam *pCmdLinePara) {
...
static ERRORTYPE loadConfig(SampleVIRVI2VOZoomConfig *pConfig,
  ...
  pConfig->mDevNum =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_DEVICE_NUMBER, 0);
  pConfig->mCaptureWidth =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_WIDTH, 0);
  pConfig->mCaptureHeight =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_HEIGHT, 0);
  pConfig->mDisplayWidth =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_WIDTH, 0);
  pConfig->mDisplayHeight =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_HEIGHT, 0);
  ...
  alogd("pConfig->mCaptureWidth=[%d], pConfig->mCaptureHeight=[%d], "
        "pConfig->mDevNum=[%d]",
        pConfig->mCaptureWidth, pConfig->mCaptureHeight, pConfig->mDevNum);
  alogd("pConfig->mDisplayWidth=[%d], pConfig->mDisplayHeight=[%d], "
        "pConfig->mDispSync=[%d], pConfig->mDispType=[%d]",
        pConfig->mDisplayWidth, pConfig->mDisplayHeight, pConfig->mDispSync,
        pConfig->mDispType);
  alogd("pConfig->mFrameRate=[%d], pConfig->mTestDuration=[%d]",
        pConfig->mFrameRate, pConfig->mTestDuration);

  pConfig->mZoomSpeed =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_ZOOM_SPEED, 0);
  pConfig->mZoomMaxCnt =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_ZOOM_MAX_CNT, 0);
  alogd("pConfig->mZoomSpeed=[%d], pConfig->mZoomMaxCnt=[%d]",
        pConfig->mZoomSpeed, pConfig->mZoomMaxCnt);
...
/**
 * This is just a calculation method of scaling parameters, we can fit a
 * specific curve as needed.
 */
static int calculateCrop(RECT_S *rect, int zoomCnt,
                         SampleVIRVI2VOZoomContext *pContext) {
  if (NULL == pContext) {
  ...
  rect->Y =
      (actualHeight - actualHeight * zoomSpeed / (zoomSpeed + zoomCnt)) / 2;
  ...
  if ((pContext->mLastRect.X != 0) && (pContext->mLastRect.X == rect->X)) {
    // alogd("[skip] X: %d, Y: %d, W: %d, H: %d ", rect->X, rect->Y,
    // rect->Width, rect->Height);
    return -1;
  }
  alogd("X: %d, Y: %d, W: %d, H: %d ", rect->X, rect->Y, rect->Width,
        rect->Height);
...
static void getActualCaptureArea(SampleVIRVI2VOZoomContext *pContext) {
  ...
  pContext->mActualCaptureWidth =
      buffer.VFrame.mOffsetRight - buffer.VFrame.mOffsetLeft;
  pContext->mActualCaptureHeight =
      buffer.VFrame.mOffsetBottom - buffer.VFrame.mOffsetTop;
...
static void *VoZoomThread(void *pThreadData) {
  ...
  VIDEO_FRAME_INFO_S buffer;
  SampleVIRVI2VOZoomContext *pContext =
      (SampleVIRVI2VOZoomContext *)pThreadData;
  if (NULL == pContext)
    return NULL;
  ...
  /**
   * if the maximum resolution supported by sensor imx335_mipi is smaller than
   * the set resolution. to avoid green screen at the edge of the display, we
   * should be correct it.
   */
  ...
  while (!pContext->mOverFlag) {
    ...
    AW_MPI_VI_GetFrame(pContext->mVIDev, 0, &buffer,
                       500); // get one frame cost 40ms
...
int main(int argc __attribute__((__unused__)),
  ...
  stAttr.format.pixelformat =
      map_PIXEL_FORMAT_E_to_V4L2_PIX_FMT(stContext.mConfigPara.mPicFormat);
  ...
  // stContext.mLayerAttr.enPixFormat =
  // map_PIXEL_FORMAT_E_to_V4L2_PIX_FMT(stContext.mConfigPara.mPicFormat);
  ...
  result =
      pthread_create(&mReadThdId, NULL, VoZoomThread, gpVIRVI2VOZoomContext);
