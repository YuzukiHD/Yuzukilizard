...
static int disp_set_addr(u32 width, u32 height, u32 addr0, u32 addr1,
                         u32 addr2) {
  config.info.fb.size[0].width = width;
  config.info.fb.size[0].height = height;
...
static void isp_server_start(int isp_id) {
   ...
   * isp_sensor_get_configs, because of sensor link will not enable until
   * s_input is called.
   */
  ...
  while (i < save_cnt) {
    ...
    if (save_cnt < 10000) {
      ...
      disp_set_addr(privCap->vfmt.format.width, privCap->vfmt.format.height,
                    privCap->buffer.planes[0].mem_phy,
                    privCap->buffer.planes[1].mem_phy,
                    privCap->buffer.planes[2].mem_phy);
      i = i % save_cnt;
    }
    ...
    // /*linux-4.9*/
    video_set_control(video, V4L2_CID_FLASH_LED_MODE_V1,
                      V4L2_FLASH_MODE_AUTO); /*linux-5.4*/
#endif
    video_set_control(video, V4L2_CID_TAKE_PICTURE, V4L2_TAKE_PICTURE_FLASH);

    isp_get_imageparams(privCap->isp_id, &isp_image_params);
    if (isp_image_params.isp_image_params.image_para.bits.flash_ok ||
        (i == 30)) {
