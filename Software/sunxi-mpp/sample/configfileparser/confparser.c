...
unsigned int GetConfParaUInt(CONFPARSER_S *pCfg, const char *key,
                             int notfound) {
  char parameterkey[128];
  ...
  if (pCfg == NULL || pCfg->pDict == NULL) {
    ...
    sprintf(parameterkey, "parameter:%s", key);
    const char *str =
        iniparser_getstring(pCfg->pDict, parameterkey, INI_INVALID_KEY);
...
const char *GetConfParaString(CONFPARSER_S *pCfg, const char *key,
                              const char *def) {
  char parameterkey[128];
