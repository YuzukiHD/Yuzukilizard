...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        {// if no available audio data, clean output buffer and set eof flag
            ...
            while(1)
            {
                while (AW_MPI_ADEC_GetFrame(stContext.mADecChn, &nFrameInfo, 500) != ERR_ADEC_BUF_EMPTY)
                {
                ...
                tmpRet = cdx_sem_down_timedwait(&stContext.mSemEofCome, 200);
                if(0 == tmpRet)
                {
                    break;
                }
                ...
                {
                    alogw("Be careful! wait EofSem fail:%d, continue", tmpRet);
                }
            }
            break;
        }
        ...
        // send stream to adec
        int nWaitTimeMs = 0; //5*1000;
_ReSendStream:
        ret = AW_MPI_ADEC_SendStream(stContext.mADecChn, &nStreamInfo, nWaitTimeMs);
        if (ret != SUCCESS)
        {
            //alogd("send StreamId[%d] with [%d]ms timeout fail[0x%x]? maybe abs is full", nStreamInfo.mId, nWaitTimeMs, ret);
            if(ERR_ADEC_BUF_FULL == ret)
            {
                while(1)
                {
                    ERRORTYPE tmpRet = AW_MPI_ADEC_GetFrame(stContext.mADecChn, &nFrameInfo, 1000);
                    if(SUCCESS == tmpRet)
                    {
                        fwrite(nFrameInfo.mpAddr, 1, nFrameInfo.mLen, stContext.mFpPcmFile);
                        AW_MPI_ADEC_ReleaseFrame(stContext.mADecChn, &nFrameInfo);
                    ...
                    {
                        alogw("get audio frame fail:0x%x, continue!", tmpRet);
        ...
        // get frame from adec
        while (AW_MPI_ADEC_GetFrame(stContext.mADecChn, &nFrameInfo, 0) == SUCCESS)
        {
            fwrite(nFrameInfo.mpAddr, 1, nFrameInfo.mLen, stContext.mFpPcmFile);
            AW_MPI_ADEC_ReleaseFrame(stContext.mADecChn, &nFrameInfo);
            nPcmDataSize += nFrameInfo.mLen;
