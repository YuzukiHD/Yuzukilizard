...
static int ParseCmdLine(int argc, char **argv,
                        SampleMotionDetectCmdLineParam *pCmdLinePara) {
...
static ERRORTYPE loadSampleMotionDetectConfig(SampleMotionDetectConfig *pConfig,
  ...
  if (pConfPath != NULL) {
    ...
    pConfig->mEncoderCount =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_EncoderCount, 0);
    pConfig->mDevNum =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_DevNum, 0);
    char *pStrPixelFormat = (char *)GetConfParaString(
        &stConfParser, SAMPLE_MOTIONDETECT_SrcPicFormat, NULL);
    ...
    pConfig->mSrcWidth =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_SrcWidth, 0);
    pConfig->mSrcHeight =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_SrcHeight, 0);
    pConfig->mSrcFrameRate =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_SrcFrameRate, 0);
    char *EncoderType = (char *)GetConfParaString(
        &stConfParser, SAMPLE_MOTIONDETECT_DstEncoderType, NULL);
    if (!strcmp(EncoderType, "H.264")) {
      ...
      alogw("unsupported venc type:%p,encoder type turn to H.264!",
            EncoderType);
      pConfig->mEncoderType = PT_H264;
    }
    pConfig->mDstWidth =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_DstWidth, 0);
    pConfig->mDstHeight =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_DstHeight, 0);
    pConfig->mDstFrameRate =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_DstFrameRate, 0);
    pConfig->mDstBitRate =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_DstBitRate, 0);
    ptr = (char *)GetConfParaString(&stConfParser,
                                    SAMPLE_MOTIONDETECT_OutputFilePath, NULL);
    strncpy(pConfig->mOutputFilePath, ptr, MAX_FILE_PATH_SIZE - 1);
    pConfig->mOutputFilePath[MAX_FILE_PATH_SIZE - 1] = '\0';

    pConfig->mOnlineEnable =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_OnlineEnable, 0);
    pConfig->mOnlineShareBufNum =
        GetConfParaInt(&stConfParser, SAMPLE_MOTIONDETECT_OnlineShareBufNum, 0);
  ...
  alogd("vipp=%d, src[%dx%d,%d], dst[%dx%d,%d], dstBitRate=%d, outputFile[%s]",
        pConfig->mDevNum, pConfig->mSrcWidth, pConfig->mSrcHeight,
        pConfig->mSrcFrameRate, pConfig->mDstWidth, pConfig->mDstHeight,
        pConfig->mDstFrameRate, pConfig->mDstBitRate, pConfig->mOutputFilePath);
  return ret;
}

static ERRORTYPE MPPCallbackWrapper(void *cookie, MPP_CHN_S *pChn,
  ...
  if (MOD_ID_VENC == pChn->mModId) {
    ...
    case MPP_EVENT_LINKAGE_ISP2VE_PARAM: {
      ...
      if (pIsp2VeParam) {
        ...
        if (ret) {
          aloge("fatal error, vipp[%d] GetIspDev failed! ret=%d",
                pContext->mViDev, ret);
          return -1;
        }
        ...
        if (ret) {
          aloge("fatal error, isp[%d] GetIsp2VeParam failed! ret=%d", mIspDev,
                ret);
          return -1;
        }
        ...
        if (mIsp2VeParam.encpp_en) {
          ...
          memcpy(&pSharpParam->mDynamicParam, &mIsp2VeParam.mDynamicSharpCfg,
                 sizeof(sEncppSharpParamDynamic));
          memcpy(&pSharpParam->mStaticParam, &mIsp2VeParam.mStaticSharpCfg,
                 sizeof(sEncppSharpParamStatic));
        } else {
          VENC_CHN_ATTR_S stVencAttr;
          memset(&stVencAttr, 0, sizeof(VENC_CHN_ATTR_S));
          AW_MPI_VENC_GetChnAttr(mVEncChn, &stVencAttr);
    ...
    case MPP_EVENT_LINKAGE_VE2ISP_PARAM: {
      ...
      if (pVe2IspParam && pContext->mConfigPara.mIspAndVeLinkageEnable) {
        ...
        alogv("update Ve2IspParam, route Isp[%d]-Vipp[%d]-VencChn[%d]", mIspDev,
              pContext->mViDev, pContext->mVeChn);

        struct enc_VencVe2IspParam mIspParam;
        memcpy(&mIspParam.mMovingLevelInfo, &pVe2IspParam->mMovingLevelInfo,
               sizeof(MovingLevelInfo));
        ret = AW_MPI_ISP_SetVe2IspParam(mIspDev, &mIspParam);
        if (ret) {
          aloge("fatal error, isp[%d] SetVe2IspParam failed! ret=%d", mIspDev,
                ret);
          return -1;
        }
        alogv("isp[%d] d2d_level=%d, d3d_level=%d", mIspDev,
              pVe2IspParam->d2d_level, pVe2IspParam->d3d_level);
...
int main(int argc, char *argv[]) {
  ...
  SampleMotionDetectContext *pContext =
      (SampleMotionDetectContext *)malloc(sizeof(SampleMotionDetectContext));
  ...
  /* parse config file. */
  if (loadSampleMotionDetectConfig(&pContext->mConfigPara, pConfigFilePath) !=
      SUCCESS) {
  ...
  if (pContext->mConfigPara.mOnlineEnable) {
    ...
    pContext->mViAttr.mOnlineShareBufNum =
  ...
  pContext->mViAttr.format.pixelformat =
      map_PIXEL_FORMAT_E_to_V4L2_PIX_FMT(pContext->mConfigPara.mSrcPicFormat);
  ...
  if (pContext->mConfigPara.mOnlineEnable) {
    ...
    pContext->mVEncChnAttr.VeAttr.mOnlineShareBufNum =
        pContext->mConfigPara.mOnlineShareBufNum;
  }
  pContext->mVEncChnAttr.VeAttr.Type = pContext->mConfigPara.mEncoderType;
  pContext->mVEncChnAttr.VeAttr.MaxKeyInterval =
      pContext->mConfigPara.mDstFrameRate;
  ...
  pContext->mVEncChnAttr.VeAttr.PixelFormat =
      pContext->mConfigPara.mSrcPicFormat;
  pContext->mVEncChnAttr.VeAttr.mColorSpace =
      pContext->mViAttr.format.colorspace;
  ...
  if (PT_H264 == pContext->mVEncChnAttr.VeAttr.Type) {
    ...
    pContext->mVEncChnAttr.VeAttr.AttrH264e.PicWidth =
        pContext->mConfigPara.mDstWidth;
    pContext->mVEncChnAttr.VeAttr.AttrH264e.PicHeight =
        pContext->mConfigPara.mDstHeight;
    ...
    pContext->mVEncChnAttr.RcAttr.mAttrH264Vbr.mMaxBitRate =
        pContext->mConfigPara.mDstBitRate;
    ...
    pContext->mVEncChnAttr.VeAttr.AttrH265e.mPicWidth =
        pContext->mConfigPara.mDstWidth;
    pContext->mVEncChnAttr.VeAttr.AttrH265e.mPicHeight =
        pContext->mConfigPara.mDstHeight;
    ...
    pContext->mVEncChnAttr.RcAttr.mAttrH265Vbr.mMaxBitRate =
        pContext->mConfigPara.mDstBitRate;
    ...
    pContext->mVEncChnAttr.VeAttr.AttrMjpeg.mPicWidth =
        pContext->mConfigPara.mDstWidth;
    pContext->mVEncChnAttr.VeAttr.AttrMjpeg.mPicHeight =
        pContext->mConfigPara.mDstHeight;
    pContext->mVEncChnAttr.RcAttr.mRcMode = VENC_RC_MODE_MJPEGCBR;
    pContext->mVEncChnAttr.RcAttr.mAttrMjpegeCbr.mBitRate =
        pContext->mConfigPara.mDstBitRate;
  }
  pContext->mVencFrameRateConfig.SrcFrmRate =
      pContext->mConfigPara.mSrcFrameRate;
  pContext->mVencFrameRateConfig.DstFrmRate =
      pContext->mConfigPara.mDstFrameRate;
  ...
  if (ret != SUCCESS) {
    aloge("fatal error! Create ViChn[%d-%d] failed", pContext->mViDev,
          pContext->mViChn);
  }
  ...
  if (ret != SUCCESS) {
    alogd("fatal error! viChn[%d-%d] enable error!", pContext->mViDev,
          pContext->mViChn);
  }
  ...
  unsigned int size = mMotionParam.hor_region_num *
                      mMotionParam.ver_region_num *
                      sizeof(VencMotionSearchRegion);
  ...
  if (strlen(pContext->mConfigPara.mOutputFilePath) > 0) {
    pContext->mOutputFileFp =
        fopen(pContext->mConfigPara.mOutputFilePath, "wb+");
    if (!pContext->mOutputFileFp) {
      aloge("fatal error! can't open file[%s]",
            pContext->mConfigPara.mOutputFilePath);
    }
  }
    ...
    if (SUCCESS == result) {
      if (stVencHeader.nLength) {
        if (pContext->mOutputFileFp != NULL) {
          fwrite(stVencHeader.pBuffer, stVencHeader.nLength, 1,
                 pContext->mOutputFileFp);
        }
      }
    } else {
  ...
  while (count < pContext->mConfigPara.mEncoderCount) {
    ...
    if ((ret = AW_MPI_VENC_GetStream(pContext->mVeChn, &stVencStream, 1000)) <
        0) {
      ...
      if (pContext->mOutputFileFp) {
        if (stVencStream.mpPack != NULL && stVencStream.mpPack->mLen0) {
          fwrite(stVencStream.mpPack->mpAddr0, 1, stVencStream.mpPack->mLen0,
                 pContext->mOutputFileFp);
        }
        if (stVencStream.mpPack != NULL && stVencStream.mpPack->mLen1) {
          fwrite(stVencStream.mpPack->mpAddr1, 1, stVencStream.mpPack->mLen1,
                 pContext->mOutputFileFp);
        }
      }
      if (pContext->bMotionSearchEnable) {
        ret = AW_MPI_VENC_GetMotionSearchResult(pContext->mVeChn,
                                                &pContext->mMotionResult);
        if (SUCCESS == ret) {
          int num = 0;
          int i = 0;
          for (i = 0; i < pContext->mMotionResult.total_region_num; i++) {
            if (pContext->mMotionResult.region[i].is_motion) {
              alogd("area_%d:[(%d,%d),(%d,%d)]", i,
                    pContext->mMotionResult.region[i].pix_x_bgn,
                    pContext->mMotionResult.region[i].pix_y_bgn,
                    pContext->mMotionResult.region[i].pix_x_end,
                    pContext->mMotionResult.region[i].pix_y_end);
              num++;
            }
          }
  ...
  if (ret < 0) {
    aloge("fatal error! Disable ViChn[%d-%d] failed", pContext->mViDev,
          pContext->mViChn);
  }
  ...
  if (ret != SUCCESS) {
    aloge("fatal error! VencChn[%d] Stop Receive Picture error!",
          pContext->mVeChn);
  }
