...
void EyeseeRecorderListener::onData(EyeseeRecorder *pMr, int what, int extra)
{
    ...
    while (1) 
    {
        ...
        {
            alogd("stream[%d] type is video extra(spspps), size[%d]", frame->mStreamId, frame->data_size);
            if(mpOwner->mRawFile)
            {
                fwrite(frame->data, 1, frame->data_size, mpOwner->mRawFile);
            }
        }
        else if(frame->stream_type == RawPacketTypeAudioExtra)
        {
            alogd("stream[%d] type is audio extra, size[%d], discard!", frame->mStreamId, frame->data_size);
        }
        else 
        {
            alogv("receive and ignore. stream id : %d-%d", frame->mStreamId, frame->stream_type);
        }
        pMr->freeOneBsFrame(frame);
    }
}
    ...
    :mRecorderListener(this)
{
    ...
    mRawFile = NULL;
    mLastCallbackPts = -1;
    mConfigPara.mEncodeFolderPath.clear();
    mConfigPara.mFilePath.clear();
    mConfigPara.mFilePath2.clear();
...
status_t SampleRecorderCallbackOutContext::loadConfig()
{
    ...
    if(ConfigFilePath.empty())
    {
        ...
        mConfigPara.mPreviewDisplayWidth = 480;
        mConfigPara.mPreviewDisplayHeight = 640;
        ...
        mConfigPara.mEncodeBitrate = 8*1024*1024;

        mConfigPara.mSubEncodeType = PT_H264;
        ...
        mConfigPara.mAudioSampleRate = 16000;
        mConfigPara.mAudioChannelNum = 1;
        mConfigPara.mAudioEncodeBitrate = 33800;
        ...
        mConfigPara.mFilePath2 = "fileCallback.mp4";
        return SUCCESS;
    }
    ...
    mConfigPara.mPreviewDisplayWidth = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_DISPLAY_WIDTH, 0); 
    mConfigPara.mPreviewDisplayHeight = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_DISPLAY_HEIGHT, 0); 
    ...
    pStrEncodeType = (char*)GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_SUB_ENCODE_TYPE, NULL); 
    if(!strcmp(pStrEncodeType, "h264"))
    {
        mConfigPara.mSubEncodeType = PT_H264;
    }
    ...
    {
        mConfigPara.mSubEncodeType = PT_H265;
    }
    else if(!strcmp(pStrEncodeType, "mjpeg"))
    {
        mConfigPara.mSubEncodeType = PT_MJPEG;
    }
    else
    {
        aloge("conf sub encode_type is [%s], forbid sub encode!", pStrEncodeType);
        mConfigPara.mSubEncodeType = PT_BUTT;
    }
    mConfigPara.mSubEncodeWidth = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_SUB_ENCODE_WIDTH, 0);
    mConfigPara.mSubEncodeHeight = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_SUB_ENCODE_HEIGHT, 0);
    mConfigPara.mSubEncodeBitrate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_SUB_ENCODE_BITRATE, 0)*1024*1024;

    pStrEncodeType = (char*)GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_AUDIO_ENCODE_TYPE, NULL); 
    ...
    {
        aloge("conf sub encode_type is [%s], forbid audio encode.", pStrEncodeType);
        mConfigPara.mAudioEncodeType = PT_BUTT;
    }
    mConfigPara.mAudioSampleRate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_AUDIO_SAMPLE_RATE, 0);
    mConfigPara.mAudioChannelNum = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_AUDIO_CHANNEL_NUM, 0);
    mConfigPara.mAudioEncodeBitrate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_AUDIO_ENCODE_BITRATE, 0);
...
int main(int argc, char *argv[])
{
    ...
    if(stContext.mConfigPara.mbPreviewEnable)
    {
        ...
        stContext.mLayerAttr.stDispRect.Width = stContext.mConfigPara.mPreviewDisplayWidth;
        stContext.mLayerAttr.stDispRect.Height = stContext.mConfigPara.mPreviewDisplayHeight;
    ...
    if(stContext.mConfigPara.mbEncodeEnable)
    {
        ...
        if(stContext.mConfigPara.mEncodeType != PT_BUTT)
        {
            ...
            {
                stVEncAttr.mAttrH264.mProfile = 1;   // 0:BL 1:MP 2:HP
                stVEncAttr.mAttrH264.mLevel = H264_LEVEL_51;
            }
            else if(PT_H265 == stVEncAttr.mType)
            {
                stVEncAttr.mAttrH265.mProfile = 0;  //0:MP
        ...
        if(stContext.mConfigPara.mSubEncodeType != PT_BUTT)
        {
            stContext.mpRecorder->setCameraProxy(stContext.mpCamera->getRecordingProxy(), cameraInfo.mMPPGeometry.mISPGeometrys[0].mScalerOutChns[1]);
            VencParameters stSubVencParam;
            alogd("setSubVideoFrameRate=[%d]", stContext.mConfigPara.mFrameRate);
            stSubVencParam.setVideoFrameRate(stContext.mConfigPara.mFrameRate);
            alogd("setSubEncodeVideoSize=[%dx%d]", stContext.mConfigPara.mSubEncodeWidth, stContext.mConfigPara.mSubEncodeHeight);
            SIZE_S stSubVideoSize;
            stSubVideoSize.Width = stContext.mConfigPara.mSubEncodeWidth;
            stSubVideoSize.Height = stContext.mConfigPara.mSubEncodeHeight;
            stSubVencParam.setVideoSize(stSubVideoSize);
            VencParameters::VEncAttr stSubVEncAttr;
            memset(&stSubVEncAttr, 0, sizeof(VencParameters::VEncAttr));
            stSubVEncAttr.mType = stContext.mConfigPara.mSubEncodeType;
            if(PT_H264 == stSubVEncAttr.mType)
            {
                stSubVEncAttr.mAttrH264.mProfile = 1;   // 0:BL 1:MP 2:HP
                stSubVEncAttr.mAttrH264.mLevel = H264_LEVEL_51;
            }
            else if(PT_H265 == stSubVEncAttr.mType)
            {
                stSubVEncAttr.mAttrH265.mProfile = 0;  //0:MP
                stSubVEncAttr.mAttrH265.mLevel = H265_LEVEL_62;
            }
            else if(PT_MJPEG == stSubVEncAttr.mType)
            {
            }
            else
            {
                aloge("fatal error! unsupport subVencType[0x%x]", stSubVEncAttr.mType);
            }
            stSubVencParam.setVEncAttr(stSubVEncAttr);
            stSubVencParam.setVideoEncoder(stContext.mConfigPara.mSubEncodeType);
            VencParameters::VEncBitRateControlAttr stSubRcAttr;
            stSubRcAttr.mVEncType = stContext.mConfigPara.mSubEncodeType;
            stSubRcAttr.mRcMode = VencParameters::VideoRCMode_CBR;
            switch(stContext.mConfigPara.mSubEncodeType)
            {
                case PT_H264:
                {
                    stSubRcAttr.mAttrH264Cbr.mBitRate = stContext.mConfigPara.mSubEncodeBitrate;
                    stSubRcAttr.mAttrH264Cbr.mMaxQp = 51;
                    stSubRcAttr.mAttrH264Cbr.mMinQp = 1;
                    break;
                }
                case PT_H265:
                {
                    stSubRcAttr.mAttrH265Cbr.mBitRate = stContext.mConfigPara.mSubEncodeBitrate;
                    stSubRcAttr.mAttrH265Cbr.mMaxQp = 51;
                    stSubRcAttr.mAttrH265Cbr.mMinQp = 1;
                    break;
                }
                case PT_MJPEG:
                {
                    stSubRcAttr.mAttrMjpegCbr.mBitRate = stContext.mConfigPara.mSubEncodeBitrate;
                    break;
                }
                default:
                {
                    aloge("fatal error! unsupport vencType[0x%x]", stContext.mConfigPara.mSubEncodeType);
                    break;
                }
            }
            stSubVencParam.setVEncBitRateControlAttr(stSubRcAttr);
            stContext.mSubVencId = 1;
            stContext.mpRecorder->setVencParameters(stContext.mSubVencId, &stSubVencParam);
            stContext.mpRecorder->bindVeVipp(stContext.mSubVencId, cameraInfo.mMPPGeometry.mISPGeometrys[0].mScalerOutChns[1]);

            //callbackOut ignore main video stream, when sub video stream is enabled.
            stContext.mCallbackStreamIdList = {1,2};
            stContext.mpRecorder->setCallbackOutStreamList(stContext.mCallbackStreamIdList);
        }
        if(stContext.mConfigPara.mAudioEncodeType != PT_BUTT)
        {
            stContext.mpRecorder->setAudioSamplingRate(stContext.mConfigPara.mAudioSampleRate);
            stContext.mpRecorder->setAudioChannels(stContext.mConfigPara.mAudioChannelNum);
            stContext.mpRecorder->setAudioEncoder(stContext.mConfigPara.mAudioEncodeType);
            stContext.mpRecorder->setAudioEncodingBitRate(stContext.mConfigPara.mAudioEncodeBitrate);
        }
        alogd("prepare()!");
