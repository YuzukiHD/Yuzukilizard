#ifndef __SAMPLE_VENC_RECREATE_H__
#define __SAMPLE_VENC_RECREATE_H__
...
typedef struct VencRecreate_Config
{
    int mVencRecreateEnable;
    int mVencRecreateTestDuration;
    int mVideoEncoderFmt;
    int mVideoFrameRate;
    int mVideoBitRate;
    int dstWidth;
    ...
    int mKeyFrameInterval;
    int mRcMode;
} VENC_RECREATE_DYNAMIC_CONFIG_S;

typedef struct SAMPLE_VENC_RECREATE_S
{
    ...
    VENC_CHN mVeChn;
    VENC_FRAME_RATE_S mstFrameRate;

    BOOL mIsVencRecreateDone;   //indicate this venc handle has finished recreate vencLib!
    VENC_RECREATE_DYNAMIC_CONFIG_S mDynamicConfig;

    pthread_t mThreadId;
    int mExitFlag;
}SAMPLE_VENC_RECREATE_S;

#endif //#define __SAMPLE_VENC_RECREATE_H__
