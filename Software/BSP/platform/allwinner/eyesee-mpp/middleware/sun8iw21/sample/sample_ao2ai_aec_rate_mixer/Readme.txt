sample_ao2ai_aec_rate_mixer测试流程：
...
读取测试参数的流程：
	sample提供了sample_ao2ai_aec_rate_mixer.conf，测试参数包括：pcm音频文件路径(dst_file)、采样率(pcm_sample_rate)、
...
从命令行启动sample_ao2ai_aec_rate_mixer的指令：
	./sample_ao2ai_aec_rate_mixer -path /mnt/extsd/sample_ao2ai/sample_ao2ai_aec_rate_mixer.conf
	"-path /mnt/extsd/sample_ao2ai/sample_ao2ai_aec_rate_mixer.conf"指定了测试参数配置文件的路径。
...
第二路音频数据文件及对应参数：
pcm_src_path_slave 
pcm_sample_rate_slave
pcm_channel_cnt_slave
pcm_bit_width_slave
pcm_frame_size_slave
