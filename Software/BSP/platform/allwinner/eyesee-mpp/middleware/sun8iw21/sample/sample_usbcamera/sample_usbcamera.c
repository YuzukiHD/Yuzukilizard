...
static SampleUsbCameraUVCFormat gFormat[MAX_FORMAT_SUPPORT_NUM][MAX_FRAME_SUPPORT_NUM] =
{
    {
        {
            .iWidth  = 1920,
...
static SampleUsbCameraContext *gpSampleUsbCameraContext = NULL;

static int OpenUVCDevice(SampleUsbCameraUVCDevice *pstUVCDev)
{
...
static void CloseUVCDevice(SampleUsbCameraUVCDevice *pUVCDev)
{
    close(pUVCDev->iFd);
}

static int OpenG2d(SampleUsbCameraUVCInfo *pUVCInfo, SampleUsbCameraConfig *pUVCConfig)
{
    int nFrmSize = 0;
...
static int G2dProc(SampleUsbCameraUVCInfo *pUVCInfo, VIDEO_FRAME_INFO_S *pSrcFrame, VIDEO_FRAME_INFO_S *pDstFrame)
{
    int ret = 0;
    g2d_blt_h blt;
...
static void CloseG2d(SampleUsbCameraUVCInfo *pUVCInfo)
{
    if (pUVCInfo->mG2dFd >= 0)
    {
...
static inline int DoUVCVideoBufProcess(SampleUsbCameraUVCDevice *pstUVCDev)
{
    SampleUsbCameraContext *pConfig = (SampleUsbCameraContext *)pstUVCDev->pPrivite;
    SampleUsbCameraUVCInfo *pUVCInfo = &pConfig->mUVCInfo;
...
//        aloge("valid frame is empty!!\n");
    ...
    SampleUsbCameraUVCOutBuf *pUvcOutBuf;
    pUvcOutBuf = list_first_entry(&pUVCInfo->mValidFrm, SampleUsbCameraUVCOutBuf, mList);
...
static int ConfigUVCBufByVE(SampleUsbCameraUVCOutBuf *pBuf, VENC_STREAM_S *pVencBuf)
{
...
static int ConfigUVCBufByVI(SampleUsbCameraUVCOutBuf *pBuf, VIDEO_FRAME_INFO_S *pFrmInfo)
{
    unsigned int offsetLen = 0;
...
{
    SampleUsbCameraContext *pContext = (SampleUsbCameraContext *)pArg;
    SampleUsbCameraUVCInfo *pUVCInfo = &pContext->mUVCInfo;
    ...
    if (MM_INVALID_CHN != pUVCInfo->iVencChn && \
        PT_H264 == pContext->mConfig.eEncoderType)
    {
        pthread_mutex_lock(&pUVCInfo->mFrmLock);
        SampleUsbCameraUVCOutBuf *pBuf = list_first_entry_or_null(&pUVCInfo->mIdleFrm, SampleUsbCameraUVCOutBuf, mList);
        if (NULL != pBuf)
        {
    ...
    while(1)
    {
        ...
        if (MM_INVALID_CHN != pUVCInfo->iVencChn && \
            PT_BUTT != pContext->mConfig.eEncoderType)
        {
        ...
        if (MM_INVALID_CHN != pUVCInfo->iVencChn && \
            PT_BUTT != pContext->mConfig.eEncoderType)
        {
            if (PT_JPEG == pContext->mConfig.eEncoderType)
            {
                pBuf->eDataType = BUF_DATA_TYPE_JPEG_STREAM;
            }
            else if (PT_MJPEG == pContext->mConfig.eEncoderType)
            {
                pBuf->eDataType = BUF_DATA_TYPE_MJPEG_STREAM;
            }
            else if (PT_H264 == pContext->mConfig.eEncoderType)
            {
...
static int InitFrameList(int iFrmNum, SampleUsbCameraContext *pContext)
{
    SampleUsbCameraUVCInfo *pInfo = &pContext->mUVCInfo;
    ...
    SampleUsbCameraUVCOutBuf *pBufTmp;
    pthread_mutex_init(&pInfo->mFrmLock, NULL);
    for (int i = 0; i < iFrmNum; i++) {
        pBufTmp = malloc(sizeof(SampleUsbCameraUVCOutBuf));
...
static void DeinitFrameList(SampleUsbCameraContext *pContext)
{
    SampleUsbCameraUVCInfo *pInfo = &pContext->mUVCInfo;

    alogd("begin to free frame list.\n");
    SampleUsbCameraUVCOutBuf *pBufTmp;
    SampleUsbCameraUVCOutBuf *pBufTmpNext;
...
static int createVipp(SampleUsbCameraUVCInfo *pUVCInfo, SampleUsbCameraConfig *pUVCConfig)
{
    ERRORTYPE eRet = SUCCESS;
...
static int createVeChn(SampleUsbCameraUVCInfo *pUVCInfo, SampleUsbCameraConfig *pUVCConfig)
{
    ERRORTYPE eRet = SUCCESS;
...
static int CreateCaptureThread(SampleUsbCameraContext *pContext)
{
    ERRORTYPE eRet = SUCCESS;
    int ret = 0;
    SampleUsbCameraUVCDevice *pUVCDev = &pContext->mUVCDev;
    ...
    {
        pContext->mConfig.eEncoderType = PT_MJPEG;
    }
    else if (V4L2_PIX_FMT_YUYV == pUVCDev->iFormat)
    {
        pContext->mConfig.eEncoderType = PT_BUTT;
    }
    else if (V4L2_PIX_FMT_H264 == pUVCDev->iFormat)
    {
        pContext->mConfig.eEncoderType = PT_H264;
    }
    pContext->mConfig.iCapWidth = pUVCDev->iWidth;
    pContext->mConfig.iCapHeight = pUVCDev->iHeight;
    pContext->mConfig.iEncWidth = pUVCDev->iWidth;
    pContext->mConfig.iEncHeight = pUVCDev->iHeight;

    alogd("capture size[%dx%d], encode size[%dx%d]", \
        pContext->mConfig.iCapWidth, pContext->mConfig.iCapHeight, \
        pContext->mConfig.iEncWidth, pContext->mConfig.iEncHeight);

    eRet = createVipp(&pContext->mUVCInfo, &pContext->mConfig);
    if (eRet)
    {
    ...
    if (PT_BUTT != pContext->mConfig.eEncoderType)
    {
        eRet = createVeChn(&pContext->mUVCInfo, &pContext->mConfig);
        if (eRet)
        {
    ...
    else
    {
        ...
        ret = OpenG2d(&pContext->mUVCInfo, &pContext->mConfig);
        if (ret)
        {
            aloge("fatal error! open g2d device fail!");
            goto _destroy_ve;
...
static int DestroyCaptureThread(SampleUsbCameraContext *pContext)
{
...
static int StartCapture(SampleUsbCameraContext *pContext)
{
    int iRet = 0;
    ERRORTYPE eRet = SUCCESS;
...
static int StopCapture(SampleUsbCameraContext *pContext)
{
...
static int UVCVideoSetFormat(SampleUsbCameraUVCDevice *pstUVCDev)
{
    int iRet;
...
static void UVCFillStreamingControl(SampleUsbCameraUVCDevice *pstUVCDev, struct uvc_streaming_control *pstCtrl, int iFmtIndex, int iFrmIndex)
{
    SampleUsbCameraUVCFormat *pFormat = &gFormat[iFmtIndex][iFrmIndex];
...
static void SubscribeUVCEvent(struct SampleUsbCameraUVCDevice *pstUVCDev)
{
...
static void UnSubscribeUVCEvent(struct SampleUsbCameraUVCDevice *pstUVCDev)
{
    struct v4l2_event_subscription stSub;
#if 0
...
static int DoUVCEventSetupClassStreaming(SampleUsbCameraUVCDevice *pstUVCDev, struct uvc_event *pstEvent, struct uvc_request_data *pstReq)
{
...
static int DoUVCEventSetupClass(SampleUsbCameraUVCDevice *pstUVCDev, struct uvc_event *pstEvent, struct uvc_request_data *pstReq)
{
...
static int DoUVCEventSetup(SampleUsbCameraUVCDevice *pstUVCDev, struct uvc_event *pstEvent, struct uvc_request_data *pstReq)
{
...
static int DoUVDReqReleaseBufs(SampleUsbCameraUVCDevice *pstUVCDev,  int iBufsNum);
static int DoUVCStreamOnOff(SampleUsbCameraUVCDevice *pstUVCDev, int iOn);
static int DoUVCEventData(SampleUsbCameraUVCDevice *pstUVCDev, struct uvc_request_data *pstReq)
{
...
//            alogd("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&commitcontrol\n");
    ...
    if (pstUVCDev->iCtrlSetCur == UVC_VS_COMMIT_CONTROL) {
        ...
        SampleUsbCameraUVCFormat *pFormat = &gFormat[pstCtrl->bFormatIndex - 1][pstCtrl->bFrameIndex - 1];
...
static int DoUVDReqReleaseBufs(SampleUsbCameraUVCDevice *pstUVCDev,  int iBufsNum)
{
    ...
    if (iBufsNum > 0) {
        ...
        pstUVCDev->pstFrames = malloc(iBufsNum * sizeof(SampleUsbCameraUVCFrame));
...
static void uac_xrun(snd_pcm_t *handle)
{
    ...
    if (ret < 0)
    {
        aloge("snd pcm status fail!");
    }
    ret = snd_pcm_status_get_state(status);
    if (SND_PCM_STATE_XRUN == ret)
    {
        alogd("snd pcm status get state is SND_PCM_STATE_XRUN");
        ret = snd_pcm_prepare(handle);
        if (ret < 0)
        {
            aloge("snd pcm prepare fail!");
            goto _exit;
        }
    }
    ret = snd_pcm_status_get_state(status);
    ...
    {
        alogd("snd pcm status get state is SND_PCM_STATE_DRAINING");
    }

    aloge("read/weitei error, status = %s", snd_pcm_state_name(snd_pcm_status_get_state(status)));
_exit:
...
static void uac_suspend(snd_pcm_t *handle)
{
    ...
    aloge("suspend, trying resume!");
    while (-EAGAIN == (ret = snd_pcm_resume(handle)))
    {
        sleep(1);
        if (ret < 0)
        {
            aloge("snd pcm resume fail! restar stream!");
            ret = snd_pcm_prepare(handle);
            if (ret < 0)
            {
                aloge("snd pcm prepare fail!");
...
static void *UACTaskProc(void *pThreadData)
{
    int nAvailNum = 0, nCount = 0;
    ERRORTYPE eRet = SUCCESS;
    AUDIO_FRAME_S stAudioFrm;
    SampleUsbCameraUACConfig *pUACCfg = (SampleUsbCameraUACConfig *)pThreadData;
    SampleUsbCameraUACAlsaConfig *pUACAlsaCfg = &pUACCfg->mUACAlsaCfg;

    alogd("UAC task proc running!");
    pUACCfg->mbTaskProcRunning = TRUE;
    while (1)
    {
        if (pUACCfg->mbTaskProcExit)
        {
            alogd("UAC task proc exit!");
            break;
        }

        memset(&stAudioFrm, 0, sizeof(AUDIO_FRAME_S));
        eRet = AW_MPI_AI_GetFrame(pUACCfg->mAIDev, pUACCfg->mAIChn, &stAudioFrm, NULL, -1);
        if (SUCCESS != eRet)
        {
            alogw("get ai frame fail!");
            continue;
        }

        nAvailNum = snd_pcm_avail(pUACAlsaCfg->mpUACHandle);
        nCount = stAudioFrm.mLen / pUACAlsaCfg->mBytesPerFrame;
        if (nAvailNum >= pUACAlsaCfg->mBufferSize / 2)
        {
            eRet = snd_pcm_writei(pUACAlsaCfg->mpUACHandle, stAudioFrm.mpAddr, nCount);
            if (-EAGAIN == eRet)
            {
                snd_pcm_wait(pUACAlsaCfg->mpUACHandle, 100);
                aloge("snd pcm writei ret[-EAGAIN] count[%d]", nCount);
            }
            else if (-EPIPE == eRet)
            {
                uac_xrun(pUACAlsaCfg->mpUACHandle);
                aloge("uac xrun");
            }
            else if (-ESTRPIPE == eRet)
            {
                uac_suspend(pUACAlsaCfg->mpUACHandle);
                aloge("uac suspend");
            }
            else if (eRet < 0)
            {
        ...
        AW_MPI_AI_ReleaseFrame(pUACCfg->mAIDev, pUACCfg->mAIChn, &stAudioFrm, NULL);
        usleep(10*1000);
    }
...
static int set_uac_audio_card_params(SampleUsbCameraUACAlsaConfig *pUACAlsaCfg, SampleUsbCameraConfig *pConfig)
{
    int ret = 0;
    snd_pcm_format_t format;

    memset(pUACAlsaCfg, 0, sizeof(SampleUsbCameraUACAlsaConfig));
    ret = snd_pcm_open(&pUACAlsaCfg->mpUACHandle, \
        pConfig->mUACAudioCard, SND_PCM_STREAM_PLAYBACK, 0);
    if (ret < 0)
    {
        pUACAlsaCfg->mpUACHandle = NULL;
        aloge("fatal error! audio card name[%s] open fail!", pConfig->mUACAudioCard);
        goto _exit;
    }

    snd_pcm_hw_params_alloca(&pUACAlsaCfg->mpUAChwParam);

    ret = snd_pcm_hw_params_any(pUACAlsaCfg->mpUACHandle, \
        pUACAlsaCfg->mpUAChwParam);
    if (ret < 0)
    {
        alogd("fatal error! unable to Fill it in with default values");
        goto _close_audio_card;
    }

    ret = snd_pcm_hw_params_set_access(pUACAlsaCfg->mpUACHandle, \
        pUACAlsaCfg->mpUAChwParam, SND_PCM_ACCESS_RW_INTERLEAVED);
    if (ret < 0)
    {
        aloge("fatal error! ubable to Interleaved mode");
        goto _close_audio_card;
    }

    switch (pConfig->mBitWidth)
    {
        case 8:
        {
        ...
        default :
        {
            aloge("fatal error! unknow pcm bit width[%d]", pConfig->mBitWidth);
            format = SND_PCM_FORMAT_UNKNOWN;
            goto _close_audio_card;
        }
    }

    ret = snd_pcm_hw_params_set_format(pUACAlsaCfg->mpUACHandle, \
        pUACAlsaCfg->mpUAChwParam, format);
	if (ret < 0)
	{
		aloge("fatal error!unable to set format");
		goto _close_audio_card;
	}

    int dir;
    unsigned int rate  = pConfig->mSampleRate;
    ret = snd_pcm_hw_params_set_rate_near(pUACAlsaCfg->mpUACHandle, \
        pUACAlsaCfg->mpUAChwParam, &rate, &dir);
    if (ret < 0)
    {
        aloge("fatal error! unable to set sampling rate!");
        goto _close_audio_card;
    }

    pUACAlsaCfg->mPeriodSize = 1024;
    snd_pcm_hw_params_set_period_size_near(pUACAlsaCfg->mpUACHandle, \
        pUACAlsaCfg->mpUAChwParam, &pUACAlsaCfg->mPeriodSize, &dir);

    pUACAlsaCfg->mBytesPerFrame = pConfig->mChannelCnt * pConfig->mBitWidth / 8;
    pUACAlsaCfg->mPeriods = 4;
    snd_pcm_hw_params_set_periods(pUACAlsaCfg->mpUACHandle, pUACAlsaCfg->mpUAChwParam, \
        pUACAlsaCfg->mPeriods, 0);

    pUACAlsaCfg->mBufferSize = pUACAlsaCfg->mPeriodSize * pUACAlsaCfg->mPeriods;
    snd_pcm_hw_params_set_buffer_size(pUACAlsaCfg->mpUACHandle, pUACAlsaCfg->mpUAChwParam, \
        pUACAlsaCfg->mBufferSize);

    ret = snd_pcm_hw_params(pUACAlsaCfg->mpUACHandle, pUACAlsaCfg->mpUAChwParam);
    if (ret < 0)
    {
        aloge("fatal error! unable to set hw parameters ret[%s]", snd_strerror(ret));
        goto _close_audio_card;
    }

    snd_pcm_hw_params_get_period_size(pUACAlsaCfg->mpUAChwParam, \
        &pUACAlsaCfg->mPeriodSize, &dir);
    alogd("snd get uac card periodsize[%d]", pUACAlsaCfg->mPeriodSize);
    snd_pcm_hw_params_get_periods(pUACAlsaCfg->mpUAChwParam, &pUACAlsaCfg->mPeriods, &dir);
    alogd("snd get uac card periods[%d]", pUACAlsaCfg->mPeriods);
    snd_pcm_hw_params_get_buffer_size(pUACAlsaCfg->mpUAChwParam, &pUACAlsaCfg->mBufferSize);
    alogd("snd get uac card buffer size[%d]", pUACAlsaCfg->mBufferSize);

    return ret;
_close_audio_card:
    snd_pcm_close(pUACAlsaCfg->mpUACHandle);
_exit:
    return ret;
}

static void ConfigAIOAttr(AIO_ATTR_S *pAIOAttr, SampleUsbCameraConfig *pConfig)
{
    pAIOAttr->u32ChnCnt = pConfig->mChannelCnt;
    pAIOAttr->enSamplerate = (AUDIO_SAMPLE_RATE_E)pConfig->mSampleRate;
    pAIOAttr->enBitwidth = (AUDIO_BIT_WIDTH_E)pConfig->mBitWidth;
}

static int CreateUAC(SampleUsbCameraUACConfig *pUACCfg, SampleUsbCameraContext *pContext)
{
    ERRORTYPE eRet = SUCCESS;
    BOOL bSuccessFlag = FALSE;
    SampleUsbCameraConfig *pConfig = &pContext->mConfig;

    memset(pUACCfg, 0, sizeof(SampleUsbCameraUACConfig));
    pUACCfg->mAIDev = 0;
    memset(&pUACCfg->mAIOAttr, 0, sizeof(AIO_ATTR_S));
    ConfigAIOAttr(&pUACCfg->mAIOAttr, pConfig);
    eRet = AW_MPI_AI_SetPubAttr(pUACCfg->mAIDev, &pUACCfg->mAIOAttr);
    if (SUCCESS != eRet)
    {
        aloge("fatal error! set ai pub attr fail!");
    }

    pUACCfg->mAIChn = 0;
    while (pUACCfg->mAIChn < AIO_MAX_CHN_NUM)
    {
        eRet = AW_MPI_AI_CreateChn(pUACCfg->mAIDev, pUACCfg->mAIChn);
        if (SUCCESS == eRet)
        {
            bSuccessFlag = TRUE;
            alogd("create ai chn[%d] success!", pUACCfg->mAIChn);
            break;
        }
        else if (ERR_AI_EXIST == eRet)
        {
            alogd("ai channel[%d] exist, find next!", pUACCfg->mAIChn);
            pUACCfg->mAIChn = MM_INVALID_CHN;
        }
        else if(ERR_AI_NOT_ENABLED == eRet)
        {
        ...
        {
            aloge("create ai channel[%d] fail! ret[0x%x]!", pUACCfg->mAIChn, eRet);
            break;
        }
    }
    if (!bSuccessFlag)
    {
        aloge("fatal error! create ai chn[%d] fail!", pUACCfg->mAIChn);
        return -1;
    }

    eRet = AW_MPI_AI_SetDevVolume(pUACCfg->mAIDev, pConfig->mAIGain);
    if (SUCCESS != eRet)
    {
        aloge("fatal error! set ai dev volume[%d] fail!", pConfig->mAIGain);
    }

    int nTmpAIGain = 0;
    AW_MPI_AI_GetDevVolume(pUACCfg->mAIDev, &nTmpAIGain);
    alogd("ai get dev volume[%d]", nTmpAIGain);

    eRet = set_uac_audio_card_params(&pUACCfg->mUACAlsaCfg, pConfig);
    if (eRet < 0)
    {
        aloge("fatal error! set uac audio card params fail!");
        return -1;
    }

    eRet = AW_MPI_AI_EnableChn(pUACCfg->mAIDev, pUACCfg->mAIChn);
    if (SUCCESS != eRet)
    {
        aloge("fatal error! ai enable fail!");
        return -1;
    }

    pthread_create(&pUACCfg->mUACTaskProcTrd, NULL, UACTaskProc, pUACCfg);

    return eRet;
}

static int DestroyUAC(SampleUsbCameraUACConfig *pUACCfg)
{
    alogd("destroy UAC");
    if (pUACCfg->mbTaskProcRunning)
    {
        pUACCfg->mbTaskProcExit = TRUE;
        pthread_join(pUACCfg->mUACTaskProcTrd, NULL);
        pUACCfg->mbTaskProcRunning = FALSE;
    }

    if (pUACCfg->mAIDev >= 0 && pUACCfg->mAIChn >= 0)
    {
        AW_MPI_AI_DisableChn(pUACCfg->mAIDev, pUACCfg->mAIChn);
        AW_MPI_AI_ResetChn(pUACCfg->mAIDev, pUACCfg->mAIChn);
        AW_MPI_AI_DestroyChn(pUACCfg->mAIDev, pUACCfg->mAIChn);
    }
    if (NULL != pUACCfg->mUACAlsaCfg.mpUACHandle)
    {
        snd_pcm_drain(pUACCfg->mUACAlsaCfg.mpUACHandle);
        snd_pcm_close(pUACCfg->mUACAlsaCfg.mpUACHandle);
    }
...
static int DoUVCStreamOnOff(SampleUsbCameraUVCDevice *pstUVCDev, int iOn)
{
    ...
    SampleUsbCameraContext *pContext = (SampleUsbCameraContext *)pstUVCDev->pPrivite;
    SampleUsbCameraUACConfig *pUACCfg = &pContext->mUACInfo;
    ...
    if (iOn) {
        ...
        if (pContext->mConfig.mbEnableUAC)
        {
            alogd("enable UAC");
            iRet = CreateUAC(pUACCfg, pContext);
            if (iRet)
            {
                aloge("fatal error! create UAC fail!");
            }
        }
...
static int DoUVCEventProcess(SampleUsbCameraUVCDevice *pstUVCDev)
{
    int iRet;
...
static ERRORTYPE LoadSampleUsbCameraConfig(SampleUsbCameraConfig *pConfig, const char *conf_path)
{
    int iRet;
    ...
    if(iRet < 0)
    {
        ...
        pConfig->mbEnableUAC = FALSE;
        goto use_default_conf;
    }

    pConfig->iUVCDev = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_UVC_DEVICE, 0);
    pConfig->iCapDev = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_VIN_DEVICE, 0);
    pConfig->iEncBitRate = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_ENC_BITRATE, 0);
    pConfig->iEncQuality = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_ENC_JPEG_QUALITY, 0);

    pConfig->mbEnableUAC = (BOOL)GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_ENABLE_UAC, 0);
    char *pTmpStr = (char *)GetConfParaString(&stConfParser, SAMPLE_USBCAMERA_KEY_UAC_DEVICE, NULL);
    strcpy(pConfig->mUACAudioCard, pTmpStr);
    pConfig->mSampleRate = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_UAC_SAMPLE_RATE, 0);
    pConfig->mBitWidth = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_UAC_BIT_WIDTH, 0);
    pConfig->mChannelCnt = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_UAC_CHANNEL_CNT, 0);
    pConfig->mAIGain = GetConfParaInt(&stConfParser, SAMPLE_USBCAMERA_KEY_UAC_AI_GAIN, 0);
use_default_conf:
    ...
    alogd("enable uac[%d] usb audio card[%s] sample rate[%d] bit width[%d] channle cnt[%d] ai gain[%d]", \
        pConfig->mbEnableUAC, pConfig->mUACAudioCard, pConfig->mSampleRate, pConfig->mBitWidth, \
        pConfig->mChannelCnt, pConfig->mAIGain);
...
static int ParseCmdLine(int argc, char **argv, SampleUsbCameraContext *pCmdLinePara)
{
    int mRet;
    int iOptIndex = 0;

    memset(pCmdLinePara, 0, sizeof(SampleUsbCameraContext));
    ...
    while (1) {
        ...
        switch (mRet) {
            ...
            case 'x':
                alogd("width is [%d]\n", atoi(optarg));
                pCmdLinePara->mConfig.iCapWidth = atoi(optarg);
                break;
            case 'y':
                alogd("height is [%d]\n", atoi(optarg));
                pCmdLinePara->mConfig.iCapHeight = atoi(optarg);
                break;
            case 'f':
                alogd("frame rate is [%d]\n", atoi(optarg));
                pCmdLinePara->mConfig.iCapFrameRate = atoi(optarg);
                break;
            case 'b':
            ...
            case 'd':
                alogd("device is [%d]\n", atoi(optarg));
                pCmdLinePara->mConfig.iUVCDev = atoi(optarg);
                break;
            case 'h':
                usage(argv[0]);
...
static void InitContext(SampleUsbCameraContext *pContext)
{
    pContext->mUVCInfo.iVippDev = MM_INVALID_DEV;
    pContext->mUVCInfo.iVencChn = MM_INVALID_CHN;
    pContext->mUVCInfo.iVencChn = MM_INVALID_CHN;
...
int main(int argc, char *argv[])
{
    ...
    alogd("SAMPLE_USBCAMERAout running!\n");
    SampleUsbCameraContext stContext;
    memset(&stContext, 0, sizeof(SampleUsbCameraContext));
    InitContext(&stContext);
    gpSampleUsbCameraContext = &stContext;
    ...
    /* parse config file. */
    if(LoadSampleUsbCameraConfig(&stContext.mConfig , stContext.mCmdLinePara.mConfigFilePath) != SUCCESS)
    {
    ...
    SampleUsbCameraUVCDevice *pstUVCDev = &stContext.mUVCDev;
    pstUVCDev->iDev = stContext.mConfig.iUVCDev;
    pstUVCDev->pPrivite = (void *)&stContext;
    pstUVCDev->bIsBulkMode = 1;
