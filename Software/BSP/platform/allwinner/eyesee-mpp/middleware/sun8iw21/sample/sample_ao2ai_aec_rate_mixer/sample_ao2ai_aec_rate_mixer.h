...
typedef struct SampleAO2AIConfig
{
    ...
    char mPcmSrcPath_slave[MAX_FILE_PATH_SIZE];
    int mSampleRate_slave;
    int mChannelCnt_slave;
    int mBitWidth_slave;
    int mFrameSize_slave;
}SampleAO2AIConfig;
...
typedef struct SampleAO2AIContext
{
    ...
    FILE *mFpPcmFile_slave;
    SampleAO2AIFrameManager mFrameManager_slave;
    pthread_mutex_t mWaitFrameLock_slave;
    int mbWaitFrameFlag_slave;
    cdx_sem_t mSemFrameCome_slave;
    cdx_sem_t mSemEofCome_slave;
    int eof_flag_slave;
    AO_CHN mAOChn_slave;
}SampleAO2AIContext;
