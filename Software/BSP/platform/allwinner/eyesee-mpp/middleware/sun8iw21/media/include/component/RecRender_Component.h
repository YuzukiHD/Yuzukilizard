...
#define MAX_REC_RENDER_PORTS (8)

typedef struct
{
    int mStreamId;  //use portId as streamId, see RecRenderComponentTunnelRequest().
    MPP_CHN_S mVeChn;
...
typedef struct RECRENDERDATATYPE {
    ...
    int mCacheStrmIds[MAX_REC_RENDER_PORTS];
    int mCacheStrmIdsCnt;
    ...
    long long last_audio_pts;

    int video_frm_cnt;  //count video0 frame count.
    int audio_frm_cnt;
