...
//  bool UnpackRTPH264(void * bufIn, int len, void ** pBufOut, int * pOutLen);
  Boolean sendRemainData(int socketNum);
  static void onSocketWriteHandler(void *context, int mask);
  void onSocketWriteHandler1();
...
private:
  ...
  std::map<int, std::stringstream> _remainDataTable;
  std::list<int> _onWriteSocketList;
