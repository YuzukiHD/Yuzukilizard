...
::PassiveServerMediaSubsession(RTPSink& rtpSink, RTCPInstance* rtcpInstance)
  : ServerMediaSubsession(rtpSink.envir()),
    fSDPLines(NULL), fRTPSink(rtpSink), fRTCPInstance(rtcpInstance), _doStartStream(NULL) {
  fClientRTCPSourceRecords = HashTable::create(ONE_WORD_HASH_KEYS);
}

void PassiveServerMediaSubsession::setStartStreamCallback(StartStreamCallback *function) {
	_doStartStream = function;
}
...
void PassiveServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  //hack by liu:
  if	(_doStartStream != NULL) {
	  _doStartStream(clientSessionId);
  }
}
