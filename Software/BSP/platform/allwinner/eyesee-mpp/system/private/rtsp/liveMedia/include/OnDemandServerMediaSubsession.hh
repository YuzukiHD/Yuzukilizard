...
public:
    typedef void (PlayCallbackFunc)(void *context);
    typedef void (StopCallbackFunc)(void *context);
    void registerPlayCallback(PlayCallbackFunc *cb, void *context);
    void registerStopCallback(StopCallbackFunc *cb, void *context);
...
protected:
    ...
    virtual void getStreamParameters(unsigned clientSessionId, netAddressBits clientAddress, Port const& clientRTPPort,
            Port const& clientRTCPPort, int tcpSocketNum, unsigned char rtpChannelId, unsigned char rtcpChannelId,
            netAddressBits& destinationAddress, u_int8_t& destinationTTL, Boolean& isMulticast, Port& serverRTPPort,
...
public:
    ...
    void sendRTCPAppPacket(u_int8_t subtype, char const* name, u_int8_t* appDependentData,
...
private:
    ...
    PlayCallbackFunc *_onPlay;
    void *_playContext;
    StopCallbackFunc *_onStop;
    void *_stopContext;
};
...
public:
    ...
    Destinations(int tcpSockNum, unsigned char rtpChanId, unsigned char rtcpChanId) :
            isTCP(True), rtpPort(0) /*dummy*/, rtcpPort(0) /*dummy*/, tcpSocketNum(tcpSockNum), rtpChannelId(rtpChanId), rtcpChannelId(
                    rtcpChanId) {
    }
...
public:
    StreamState(OnDemandServerMediaSubsession& master, Port const& serverRTPPort, Port const& serverRTCPPort,
            RTPSink* rtpSink, BasicUDPSink* udpSink, unsigned totalBW, FramedSource* mediaSource, Groupsock* rtpGS,
            Groupsock* rtcpGS);
    virtual ~StreamState();

    void startPlaying(Destinations* destinations, unsigned clientSessionId, TaskFunc* rtcpRRHandler,
            void* rtcpRRHandlerClientData, ServerRequestAlternativeByteHandler* serverRequestAlternativeByteHandler,
            void* serverRequestAlternativeByteHandlerClientData);
    void pause();
    void sendRTCPAppPacket(u_int8_t subtype, char const* name, u_int8_t* appDependentData,
            unsigned appDependentDataSize);
    ...
    static void sendSRReport(void *stream_state);
