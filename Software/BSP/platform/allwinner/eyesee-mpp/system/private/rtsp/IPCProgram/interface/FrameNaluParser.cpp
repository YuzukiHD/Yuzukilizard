...
FrameNaluParser::FrameNaluParser(uint8_t *buffer, int32_t size)
    : _type(ManagerBuffer),
      _buffer(buffer),
      _bufferSize(size),
...
FrameNaluParser::FrameNaluParser(H264FrameParserType type)
    : _type(type),
      _buffer(NULL),
      _bufferSize(0),
      _currStartPtr(NULL),
...
FrameNaluParser::~FrameNaluParser()
{
...
void FrameNaluParser::setBuffer(uint8_t *buffer, uint32_t size)
{
    if (_type == ManagerBuffer) {
        free(_buffer);
    }
...
void FrameNaluParser::resetParser()
{
...
bool FrameNaluParser::getNextNaluBuffer(uint8_t *&buffer, uint32_t &size, bool &isLastNalu)
{
    bool result = true;
...
void FrameNaluParser::dumpStartcode()
{
...
bool FrameNaluParser::findStartcode(uint8_t *start, uint8_t *end, uint8_t *&startcode, uint32_t &startcodeSize)
{
