...
void OnDemandServerMediaSubsession::registerPlayCallback(PlayCallbackFunc *cb, void *context) {
    _onPlay = cb;
    _playContext = context;
}

void OnDemandServerMediaSubsession::registerStopCallback(StopCallbackFunc *cb, void *context) {
    _onStop = cb;
...
{
    if (_onPlay) _onPlay(_playContext);

    StreamState* streamState = (StreamState*)streamToken;
    Destinations* destinations = (Destinations*)(fDestinationsHashTable->Lookup((char const*)clientSessionId));
    if (streamState != NULL) {
        streamState->startPlaying(destinations, clientSessionId, rtcpRRHandler, rtcpRRHandlerClientData,
...
//        envir().taskScheduler().scheduleDelayedTask(0, StreamState::sendSRReport, streamState);
    }
}
...
void OnDemandServerMediaSubsession::pauseStream(unsigned /*clientSessionId*/, void* streamToken)
{
    ...
    if(_onStop) _onStop(_stopContext);
}
...
float OnDemandServerMediaSubsession::getCurrentNPT(void* streamToken)
{
    do {
        ...
        return streamState->startNPT() +
               ...
               (rtpSink->mostRecentPresentationTime().tv_usec - rtpSink->initialPresentationTime().tv_usec) /
                 1000000.0f;
    } while (0);
...
void OnDemandServerMediaSubsession::getRTPSinkandRTCP(void* streamToken, RTPSink const*& rtpSink,
...
void OnDemandServerMediaSubsession::closeStreamSource(FramedSource* inputSource) { Medium::close(inputSource); }
...
void OnDemandServerMediaSubsession::sendRTCPAppPacket(u_int8_t subtype, char const* name, u_int8_t* appDependentData,
                                                           ...
                                                           unsigned estBitrate)
{
    ...
    unsigned sdpFmtSize = strlen(sdpFmt) + strlen(mediaType) + 5 /* max short len */ + 3 /* max char len */
                          ...
                          + strlen(rtpmapLine) + strlen(rtcpmuxLine) + strlen(rangeLine) + strlen(auxSDPLine) +
                          strlen(trackId());
...
//    Hack: delay send SR report, change to VSplayer
//    if (fRTCPInstance != NULL) {
...
void StreamState::sendSRReport(void *streamState)
{
    StreamState *s = (StreamState*)streamState;

    if (s->fRTCPInstance != NULL) {
        // Hack: Send an initial RTCP "SR" packet, before the initial RTP packet, so that receivers will (likely) be able to
        // get RTCP-synchronized presentation times immediately:
        s->fRTCPInstance->sendReport();
    }
}
...
void StreamState::sendRTCPAppPacket(u_int8_t subtype, char const* name, u_int8_t* appDependentData,
                                    unsigned appDependentDataSize)
{
    if (fRTCPInstance != NULL) {
