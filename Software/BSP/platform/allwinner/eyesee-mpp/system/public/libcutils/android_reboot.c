...
int android_reboot(int cmd, int flags, char *arg)
{
    ...
    switch (cmd) {
        case ANDROID_RB_RESTART:
			klog_write(1, "android_reboot cmd = ANDROID_RB_RESTART flags = %d",flags);
            ret = reboot(RB_AUTOBOOT);
            break;

        case ANDROID_RB_POWEROFF:
			klog_write(1, "android_reboot cmd = ANDROID_RB_POWEROFF flags = %d",flags);
        ...
        case ANDROID_RB_RESTART2:
			...
			klog_write(1, "android_reboot cmd = ANDROID_RB_RESTART2 flags = %d",flags);
            write_misc(arg);
			ret = reboot(RB_AUTOBOOT);
            break;
