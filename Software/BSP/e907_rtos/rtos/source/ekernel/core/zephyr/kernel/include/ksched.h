...
static __attribute__((noinline)) int _is_preempt(struct k_thread *thread)
{
 ...
 */
static __attribute__((noinline)) int _must_switch_threads(void)
{
...
static inline struct k_thread *_find_first_thread_to_unpend(_wait_q_t *wait_q, struct k_thread *from)
{
#ifdef CONFIG_SYS_CLOCK_EXISTS
