...
static inline struct ieee802154_fcf_seq *validate_fc_seq(u8_t *buf, u8_t **p_buf)
{
...
static inline struct ieee802154_address_field *validate_addr(u8_t *buf, u8_t **p_buf,
...
static inline bool validate_beacon(struct ieee802154_mpdu *mpdu, u8_t *buf, u8_t length)
{
...
static inline bool validate_mac_command(struct ieee802154_mpdu *mpdu, u8_t *buf, u8_t length)
{
...
static inline enum ieee802154_addressing_mode get_dst_addr_mode(struct net_linkaddr *dst, bool *broadcast)
{
    if (!dst->addr)
    {
        NET_DBG("Broadcast destination");
