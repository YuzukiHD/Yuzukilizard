...
static void _main(void *unused1, void *unused2, void *unused3)
{
    ...
    extern int init_sem_module(struct device * dev);
    init_sem_module(NULL);
    extern int init_mutex_module(struct device * dev);
    init_mutex_module(NULL);
...
static void prepare_multithreading(struct k_thread *dummy_thread)
{
    ...
    for (ii = 0; ii < K_NUM_PRIORITIES; ii++)
    {
...
FUNC_NORETURN void _Cstart(void)
{
    ...
    memset(dummy_thread, 0x00, sizeof(struct k_thread));
#endif
    ...
    extern int init_static_pools(struct device * unused);
    init_static_pools(NULL);

    extern void zephyr_bsp_init(void);
    zephyr_bsp_init();
