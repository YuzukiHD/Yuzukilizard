...
#define _NOT_PRINT_FMT (1)
  ...
  for (s = 0; s < dim_num; s++) {
    ...
    count += snprintf(&buf[count], buf_sz - count, fmt, shape[s]);
  }
  buf[count - 1] = 0;
  return count;
}

static vip_float_t uint8_to_fp32(vip_uint8_t val, vip_int32_t zeroPoint,
                                 vip_float_t scale) {
...
#define _DUMP_SHAPE_LENGTH 128
  ...
  for (i = 0; i < network_items->output_count; i++) {
    ...
    shape_to_string(buf_param.sizes, buf_param.num_of_dims, shape,
                    _DUMP_SHAPE_LENGTH, vip_false_e);
    ...
    printf("data_format=%d buff_size=%d\n", buf_param.data_format,
           buff_size); // VIP_BUFFER_FORMAT_UINT8=2
